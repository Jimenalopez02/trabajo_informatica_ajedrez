...
void Tablero::inicializaTablero() {
    ...
    for (int i = 0; i < COLUMNAS_TABLERO; i++) {
        Coordenada posBlancas = coordenadaSobreTablero[1 * COLUMNAS_TABLERO + i];
        Coordenada posNegras = coordenadaSobreTablero[5 * COLUMNAS_TABLERO + i];

        //casillas[1][i] = new Peon(ColorBlancas, 1, i, posBlancas);
        //casillas[6][i] = new Peon(ColorNegras, 6, i, posNegras);
        casillas[1][i] = new Peon(ColorBlancas, 1, i);
        casillas[6][i] = new Peon(ColorNegras, 6, i);
    }
}
void Tablero::dibuja() {
...
void Tablero::mostrarMovimientosPeon(int fila, int columna, bool turnoBlancas) const {
    if (fila < 0 || fila >= 8 || columna < 0 || columna >= 10) {
    ...
    if (p->getTipo() != TipoPieza::Peon) {
        std::cout << "La pieza seleccionada no es un peón.\n";
        return;
    }

    std::vector<Casilla> movimientos = p->getMovimientosPermitidos(fila, columna, turnoBlancas);
    if (movimientos.empty()) {
        std::cout << "Este peón no tiene movimientos disponibles.\n";
    }
    else {
        std::cout << "Movimientos disponibles para el peón en (" << fila << ", " << columna << "):\n";
        for (const Casilla& mov : movimientos) {
