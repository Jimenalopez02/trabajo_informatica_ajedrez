// Tablero.cpp actualizado con coordenadas en píxeles y lógica existente
#include "Tablero.h"
#include "Peon.h"
#include "Caballo.h"
#include "Torre.h"
...
void Tablero::dibujar() const {
    // 1. Activar texturas y desactivar luces
    glEnable(GL_TEXTURE_2D);
    glDisable(GL_LIGHTING);

    // 2. Cargar textura del tablero
    GLuint textura = ETSIDI::getTexture("imagenes/tablero_etsidi.png").id;
    glBindTexture(GL_TEXTURE_2D, textura);

    // 3. Dibujar el tablero en el área alineada con las piezas
    glBegin(GL_QUADS);
    glColor3f(1, 1, 1);
    ...
    // 4. Desactivar textura, volver a luces
...
std::vector<Casilla> Tablero::obtenerMovimientosPermitidos(int fila, int columna, bool turnoBlancas) const {
    ...
    // ✅ Evitar devolver movimientos si no es el turno del color de la pieza
